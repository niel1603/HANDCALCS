...
E = 29000 * ksi
# Beam and column
F_y_beam_column = 50 * ksi
F_u_beam_column = 65 * ksi
# Plate
F_y_plate = 50 * ksi
F_u_plate = 65 * ksi
# Stiffener
F_y_stiffener = 50 * ksi
F_u_stiffener = 65 * ksi
# Bolt
F_nt_bolt = 113 * ksi
F_nv_bolt = 68 * ksi

# Bolt pretension force
T_b = 102 * kip # For grade A490 bolt according to AISC Specification Table J3.1
...
# Geometry properties
# Beam
d_b = 24.1 * inch # web height
b_bf = 9.02 * inch # flange width
t_bw = 0.470 * inch # web thick
t_bf = 0.770 * inch # flange thick

# Column
d_c = 14.2 * inch # web height
b_cf = 14.6 * inch # flange width
t_cw = 0.485 * inch # web thick
t_cf = 0.780 * inch # flange thick
k_des = 1.38 * inch # distance from outer face of flange to web toe of fillet used for design

# End plate geometric properties
b_p = 9 * inch # width plate
t_p = 7/8  * inch # thick plate

# Bolt geometric properties
d_bolt = (1+1/4) * inch # bolt diameter
clearance = 1/8 * inch # bolt clearance
d_hole = (d_bolt + clearance) # hole diameter

# Bolt configuration geometric properties
g = (5+3/4) * inch # width bolt hole 
p_ext = (4+1/2) * inch # distance from top side of plate to top of beam 
d_e = (1+3/4) * inch # distance from top side of plate to bolt hole
p_fo = (2+3/4) * inch # distance from top side of beam to bolt hole
p_fi = 2 * inch # distance from inner top side of beam (bottom of flange) to inner bolt hole
d_p = (33+1/8) * inch # height bolt hole

# Stiffener geometric properties
t_s = 1/2 * inch # thick stiffener
L_st = 8 * inch # stiffener length
h_st = (4+1/2) * inch # thick stiffener heigth
...
# Substitution
print("Substitution:")
print(f"  {b_p} ≤ {b_bf} + max({t_p}, {1 * inch})")

# Result
print("Result:")
b_p_limit = b_bf + max(t_p, 1 * inch)
...
slenderness_limit = 0.56 * sqrt(E / F_y_stiffener)
...
# Result
t_p_req = sqrt(M_u_eq / (gamma_r * theta_b * F_y_plate * Y_p))
...
# Result
a_part1 = (3.62 * (t_p / d_bolt)**3) * inch
a_part2 = 0.085 * inch 
a = a_part1 - a_part2
...
Q_max1_part2 = sqrt(F_y_plate**2 - 3*(F_1_prime/(w1_prime*t_p))**2)
...
Q_max2_part2 = sqrt(F_y_plate**2 - 3*(F_2_prime/(w1_prime*t_p))**2)
